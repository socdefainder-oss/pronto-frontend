...
export default function TrackingPage({ params }: { params: { id: string } }) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-pink-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md text-center">
          ...
          <Link href="/" className="inline-block px-6 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700">
            Voltar para início
          </Link>
        </div>
      </div>
  ...
  const statusSteps = [
    ...
    { key: "delivered", label: "Finalizado", icon: "🎉" },
  ];
  ...
  const getPaymentMethodLabel = (method: string | null) => {
    const methods: Record<string, string> = {
      ...
      cash: "Dinheiro",
      credit_card_delivery: "Crédito na entrega",
  ...
  const getPaymentStatusLabel = (status: string) => {
    const statuses: Record<string, { label: string; color: string }> = {
      pending: { label: "Aguardando pagamento", color: "text-yellow-600 bg-yellow-100" },
    ...
    return statuses[status] || { label: status, color: "text-gray-600 bg-gray-100" };
  };

  const handleWhatsApp = () => {
    const phone = order.restaurant.phone.replace(/\D/g, "");
    const message = `Olá! Tenho uma dúvida sobre o pedido #${order.orderNumber}`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link href={`/r/${order.restaurant.slug}`} className="flex items-center gap-2 text-gray-700 hover:text-gray-900">
          ...
          {order.restaurant.logoUrl && (
            <img src={order.restaurant.logoUrl} alt={order.restaurant.name} className="h-8 object-contain" />
          )}
        </div>
      </header>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-8 mb-6">
          ...
          <div className="text-center mb-8">
            <div className="inline-block px-6 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 rounded-full mb-4">
              <span className="text-white font-bold text-lg">Pedido #{order.orderNumber}</span>
            </div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">{order.restaurant.name}</h1>
            <p className="text-gray-600">Pedido realizado em {formatDate(order.createdAt)}</p>
          </div>
                ...
                {statusSteps.map((step, index) => {
                  ...
                  return (
                    <div key={step.key} className="flex flex-col items-center relative z-10" style={{ flex: 1 }}>
                      <div
                        className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center text-xl md:text-2xl transition-all duration-300 ${
                          isCompleted
          ...
          {/* Payment Status */}
          <div className="mb-6">
            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full font-bold ${paymentStatusInfo.color}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
              </svg>
              {paymentStatusInfo.label} • {getPaymentMethodLabel(order.paymentMethod)}
            </div>
          </div>
          ...
          <button
            ...
            className="w-full py-4 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-bold rounded-xl shadow-lg flex items-center justify-center gap-3 transition-all"
          >
        ...
        <div className="bg-white rounded-3xl shadow-xl p-6 md:p-8 mb-6">
              ...
              <div key={index} className="flex items-start gap-4 pb-4 border-b border-gray-100 last:border-0">
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between">
                    ...
                    <p className="font-bold text-emerald-600">{formatCurrency(item.priceCents * item.quantity)}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="mt-6 pt-6 border-t-2 border-gray-200 space-y-2">
            ...
            <div className="flex justify-between text-xl font-bold text-gray-900 pt-2">
              <span>Total</span>
              <span className="text-emerald-600">{formatCurrency(order.totalCents)}</span>
            </div>
          </div>
        </div>
        ...
        {order.address && (
          <div className="bg-white rounded-3xl shadow-xl p-6 md:p-8 mb-6">
            ...
            {order.address.reference && (
              <p className="text-gray-600 text-sm mt-1">Referência: {order.address.reference}</p>
            )}
          </div>
        )}
