...
export default function CouponsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function loadCoupons() {
    try {
      ...
      const response = await fetch(`${API_URL}/api/coupons/restaurant/${restaurantId}`, {
  ...
  function openEditModal(coupon: Coupon) {
    ...
    setFormData({
      ...
      minOrderCents: coupon.minOrderCents.toString(),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      minOrderCents: parseInt(formData.minOrderCents),
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-6 text-lg font-medium text-gray-700">Carregando cupons...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-8">
        {coupons.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-xl border-2 border-gray-200 p-16 text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-800 mb-3">Nenhum cupom criado</h3>
            <p className="text-gray-600 mb-6">Crie cupons de desconto para atrair mais clientes!</p>
            <button
              onClick={openCreateModal}
            ...
            {coupons.map((coupon) => (
              <div
                ...
                className={`bg-white rounded-2xl shadow-xl border-2 p-6 ${
                  coupon.isActive ? "border-emerald-300" : "border-gray-300 opacity-60"
                }`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="inline-block px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold rounded-xl text-xl mb-3">
                      {coupon.code}
                    </div>
                    <div className="flex items-center gap-2 text-gray-700">
                      {coupon.type === "percentage" ? (
                        <>
                          ...
                          <span className="font-bold text-2xl text-emerald-600">{coupon.value}%</span>
                        ...
                        <>
                          ...
                          <span className="font-bold text-2xl text-emerald-600">R$ {(coupon.value / 100).toFixed(2)}</span>
                          <span>de desconto</span>
                        </>
                      )}
                    </div>
                ...
                <div className="space-y-2 mb-4 text-sm text-gray-600">
                  {coupon.minOrderCents > 0 && (
                    <div className="flex items-center gap-2">
                      ...
                      Mínimo: R$ {(coupon.minOrderCents / 100).toFixed(2)}
                    </div>
                  )}
                  {coupon.maxUses && (
                    <div className="flex items-center gap-2">
                  ...
                  {coupon.startDate && (
                    <div className="flex items-center gap-2">
                      ...
                      Início: {new Date(coupon.startDate).toLocaleDateString("pt-BR")}
                    </div>
                  )}
                  {coupon.endDate && (
                    <div className="flex items-center gap-2">
                      ...
                      Fim: {new Date(coupon.endDate).toLocaleDateString("pt-BR")}
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2 border-2 border-gray-200 text-gray-700 font-semibold rounded-xl hover:border-blue-500 hover:bg-blue-50 hover:text-blue-700 transition"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 uppercase"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const val = formData.type === "fixed"
                        ? Math.round(parseFloat(e.target.value || "0") * 100).toString()
                        : e.target.value;
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.01"
                    value={(parseInt(formData.minOrderCents) / 100).toFixed(2)}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        minOrderCents: Math.round(parseFloat(e.target.value || "0") * 100).toString(),
                      })
                    }
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500"
                  />
